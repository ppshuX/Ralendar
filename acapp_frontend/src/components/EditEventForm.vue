  ...
  methods: {
    loadEvent() {
      if (this.eventId) {
        ...
        if (this.event) {
          ...
          this.form.start_time = this.formatDateTimeLocal(this.event.start_time)
          this.form.end_time = this.formatDateTimeLocal(this.event.end_time)
          this.form.location = this.event.location || ''
          this.form.description = this.event.description || ''
          this.form.reminder_minutes = this.event.reminder_minutes || 15
        }
      }
    },
    ...
    async submitForm() {
      ...
      try {
        ...
        const success = await this.$store.dispatch('updateEvent', {
          id: this.eventId,
          eventData
        })
